...
from aind_behavior_dynamic_foraging_curricula.cli import _KNOWN_CURRICULA


def main(root: str = "./schema", dry_run: bool = False) -> None:
    pathlib.Path(root).mkdir(parents=True, exist_ok=True)
    for curriculum in _KNOWN_CURRICULA:
        module = importlib.import_module(f"aind_behavior_dynamic_foraging_curricula.{curriculum}")
